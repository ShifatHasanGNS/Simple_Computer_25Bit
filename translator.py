...
def validate_instructions(instructions: list, vars: list, labels: list) -> bool:
  for instruction in instructions:
    ...
    if opcode in ['set', 'clr']:
      if instruction_parts[1].isdecimal() == True:
        return True

    if opcode in BinaryOpCodes:
      if address[0] == '#':
        if address[1] == 'd':
          ...
          if address[1:].isdecimal() == False:
            print(f"[ ERROR : Invalid Instruction ] --> '{instruction}'")
            return False

      elif address not in vars and address not in labels:
...
def translate(source_file: str, output_file: str):
    ...
    if 'hlt' not in instructions or instructions[-1] != 'hlt':  # insert 'hlt' instruction, if not present
